...
router.post('/entries', authenticate, async (req, res) => {
  const { title, url, username, password, notes, tags, strength } = req.body || {};
  if (!title || !username || !password) {
    return res.status(400).json({ status: 'error', message: 'Title, Username, and Password are required' });
  }
  try {
